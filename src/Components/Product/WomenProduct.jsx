...
import { getwomensProduct } from "../../Server/Apis";
import { getwomensEthinicProduct } from "../../Server/Apis";
import "./Product.css";
...
export default function WomenProduct() {
  ...
  const getProducts = async () => {
    ...
    try {
      const data = await getwomensProduct();
      const Ethinic = await getwomensEthinicProduct();
      setIsLoading(true);
      console.log(data.data);
      setWomen(Ethinic.data);
      setData(data.data,Ethinic.data);
      setIsLoading(false);
  ...
  const handleHightoLow = (e) => {
    ...
    if (option === "hightolow") {
      const updatelist = [...data].sort(
        (a, b) => b.discounted_price - a.discounted_price
      );
      const updatelist2 = [...women].sort(
        (a, b) => b.discounted_price - a.discounted_price
      );
      setData(updatelist);
      setWomen(updatelist2)
    }
    ...
    if (option === "discount") {
      const updatelist = [...data].sort(
        (a, b) =>
          Math.floor(
            ((b.original_price - b.discounted_price) / b.original_price) * 100
          ) -
          Math.floor(
            ((a.original_price - a.discounted_price) / a.original_price) * 100
          )
      );
      setData(updatelist);
    }
      ...
      <div className="product">
        <div className="SideBar">
          <ProductSidebar
            ...
            product="Sharee For Women"
            diff1="Western Drerss"
            diff2="Cotton Sharee"
            diff3="Dress"
          />
          <Box sx={{ maxWidth: 200 }}>
